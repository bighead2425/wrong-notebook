...
# canvas has no prebuilt binary for Alpine/musl, so we install build tools + system libs needed by node-gyp
RUN apk add --no-cache libc6-compat python3 make g++ pkgconfig \
    pixman-dev cairo-dev pango-dev libjpeg-turbo-dev giflib-dev librsvg-dev libpng-dev
WORKDIR /app
