...
- **M0 数据底座**：打印实例、回收件、复习排程接线、错因分类字段、批次与匣子状态；migration 规范。
- **M3 卡片版面**：B5 双面固定版式、身份条、十字象限、遮挡锚点、定位角标、按纸型分派、打印实例递增。
- **验收**：真实题库抽 20 题 → 打成 B5 双面 PDF → 用尺子量：正反各一页、装得下、字高 ≥2.5mm、象限与锚点位置正确。
